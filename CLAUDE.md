...
API REST de referencia (dominio: Produtos) construida com Fastify 5, TypeScript, Prisma 7 e Better-Auth. Roda em Node.js 24.x com pnpm 10.30.0 (ambos obrigatorios via `engine-strict`).

## Comandos

```bash
# Iniciar servidor de desenvolvimento (hot-reload, porta padrao 8080 via env PORT)
pnpm dev
...
Build: `pnpm build` (roda `prisma generate && tsc`, compila para `./dist`). Nao ha script de teste configurado ainda.
...
- **Use Cases** (`src/usecases/`) — Classes de logica de negocio. Recebem DTOs, mapeiam o retorno do Prisma para um `OutputDto` proprio (nunca retornam o model direto). Usam transacoes Prisma quando a operacao exigir atomicidade. Uma classe por caso de uso.
...
- **Zod 4** para validacao (usa padrao `z.object()`)
- **CORS** permite a origem definida em `WEB_APP_BASE_URL` (default `http://localhost:3000`) com credentials
- Variaveis de ambiente: `PORT`, `DATABASE_URL`, `BETTER_AUTH_SECRET`, `API_BASE_URL`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `WEB_APP_BASE_URL`, `NODE_ENV`
